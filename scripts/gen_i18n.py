...
EN = {
    ...
    "meta_desc": "tautiu (透抽) is Taiwanese for squid. One little squid, ten busy arms — indie Mac & iPhone apps: DeepSRT, DeepSRT Transcribe, Foldic, Recilic, Ripplic, Mergic, Nookic, Lossic, OpenAB Connect, OpenAB Remote.",
    ...
    one little squid, ten busy arms, each holding an app
...
LANGS = {
    "zh": {
        ...
        "meta_desc": "tautiu（透抽）是台語的魷魚。一隻小透抽、十隻忙碌的觸手 — 獨立開發的 Mac 與 iPhone App：DeepSRT、DeepSRT Transcribe、Foldic、Recilic、Ripplic、Mergic、Nookic、Lossic、OpenAB Connect、OpenAB Remote。",
    ...
    "ja": {
        ...
        "meta_desc": "tautiu（透抽）は台湾語でイカのこと。小さなイカと十本の忙しい腕 — Mac と iPhone のためのインディーアプリ：DeepSRT、DeepSRT Transcribe、Foldic、Recilic、Ripplic、Mergic、Nookic、Lossic、OpenAB Connect、OpenAB Remote。",
    ...
    "ko": {
        ...
        "meta_desc": "tautiu(透抽)는 대만어로 오징어. 작은 오징어와 열 개의 바쁜 다리 — Mac과 iPhone을 위한 인디 앱: DeepSRT, DeepSRT Transcribe, Foldic, Recilic, Ripplic, Mergic, Nookic, Lossic, OpenAB Connect, OpenAB Remote.",
...
for code, t in LANGS.items():
    ...
    for key in ("tagline", "story", "deepsrt_transcribe", "deepsrt", "foldic", "recilic",
                "ripplic", "mergic", "nookic", "lossic", "openab_connect", "openab_remote",
