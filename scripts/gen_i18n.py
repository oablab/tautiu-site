...
EN = {
    ...
    "meta_desc": "tautiu (透抽) is Taiwanese for squid. One little squid, seven busy tentacles — indie Mac & iPhone apps: DeepSRT, DeepSRT Transcribe, Foldic, Recilic, Ripplic, Mergic, Nookic.",
    "og_title": "tautiu — a tiny indie app studio",
    "og_desc": "One little squid, seven busy tentacles. Indie Mac & iPhone apps made with care.",
    "tagline": "a tiny indie app studio",
    "story": """<span class="zh">tautiu（透抽）</span> is Taiwanese for squid —
    one little squid, seven busy tentacles, each holding an app
    ...
    "deepsrt_transcribe": "Fully offline transcription with Apple's on-device speech models — a 6-minute video in just 10 seconds — then refined by Gemini and auto-corrected with Google Search grounding.",
...
LANGS = {
    "zh": {
        ...
        "meta_desc": "tautiu（透抽）是台語的魷魚。一隻小透抽、七隻忙碌的觸手 — 獨立開發的 Mac 與 iPhone App：DeepSRT、DeepSRT Transcribe、Foldic、Recilic、Ripplic、Mergic、Nookic。",
        "og_title": "tautiu — 小小的獨立 App 工作室",
        "og_desc": "一隻小透抽、七隻忙碌的觸手。用心打造的獨立 Mac 與 iPhone App。",
    ...
    每隻都抓著一個為你的 Mac 和 iPhone 打造的 App。""",
        ...
        "deepsrt_transcribe": "Apple 語音模型完全本地運算，影像聲音全部離線處理 — 6 分鐘影片只要 10 秒 — 再由最強 Gemini 模型優化語言，Ground with Google Search 自動校正。",
    ...
    "ja": {
        ...
        "meta_desc": "tautiu（透抽）は台湾語でイカのこと。小さなイカと七本の忙しい腕 — Mac と iPhone のためのインディーアプリ：DeepSRT、DeepSRT Transcribe、Foldic、Recilic、Ripplic、Mergic、Nookic。",
        "og_title": "tautiu — 小さなインディーアプリスタジオ",
        "og_desc": "小さなイカと七本の忙しい腕。心を込めてつくった Mac と iPhone のアプリ。",
    ...
    Mac と iPhone のためのアプリをひとつずつ。""",
        ...
        "deepsrt_transcribe": "Apple の音声モデルで完全オンデバイス処理、映像も音声もすべてオフライン — 6分の動画をわずか10秒で。さらに最強の Gemini が言語を最適化し、Google 検索グラウンディングで自動補正。",
    ...
    "ko": {
        ...
        "meta_desc": "tautiu(透抽)는 대만어로 오징어. 작은 오징어와 일곱 개의 바쁜 다리 — Mac과 iPhone을 위한 인디 앱: DeepSRT, DeepSRT Transcribe, Foldic, Recilic, Ripplic, Mergic, Nookic.",
        "og_title": "tautiu — 작은 인디 앱 스튜디오",
        "og_desc": "작은 오징어와 일곱 개의 바쁜 다리. 정성껏 만든 Mac & iPhone 앱.",
        "tagline": "작은 인디 앱 스튜디오",
        "story": """<span class="zh">tautiu（透抽）</span>는 대만어로 오징어 —
    작은 오징어 한 마리가 일곱 개의 바쁜 다리로,
    Mac과 iPhone을 위한 앱을 하나씩 들고 있어요.""",
        "deepsrt": "어떤 YouTube 영상도 즉시 AI 요약과 클릭 가능한 자막으로 — Mac 네이티브로.",
        "deepsrt_transcribe": "Apple 음성 모델로 완전 온디바이스 처리, 영상과 음성 모두 오프라인 — 6분 영상을 단 10초에. 최강 Gemini 모델의 언어 최적화와 Google 검색 그라운딩으로 자동 교정까지.",
...
for code, t in LANGS.items():
    ...
    for key in ("tagline", "story", "deepsrt_transcribe", "deepsrt", "foldic", "recilic",
                "ripplic", "mergic", "nookic", "ribbon_internal", "ribbon", "footer"):
